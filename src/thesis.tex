...
\usepackage{xcolor}
\usepackage{colortbl}
% Table shorthands used in results and appendix
\newcommand{\na}{$\circ$}       % cell: not run / no data
\newcommand{\pt}{$^\dagger$}    % cell: partial run
\newcommand{\pdd}{$^\ddagger$}  % cell: early-training transient / unstable checkpoint
\newcommand{\pst}{$^*$}         % cell: training still in progress (live checkpoint)
...
%%%% Font setup: use Libertinus for text and New Computer Modern for math
...
\newsavebox{\headbox}\savebox{\headbox}{
    ...
    \raisebox{-1ex}{\includegraphics[width=0.1\textwidth]{Figures/vgu-logo.pdf}}
}    
...
\usepackage{tikz}
\usetikzlibrary{math,positioning,arrows.meta,shapes.geometric,fit,backgrounds,calc}
...
\fancyhead[c]{\hspace*{0.15\textwidth}\footnotesize\rightmark}
%\fancyhead[C]{\usebox\headbox}                        % Zentrierte Kopfzeile
\fancyhead[R]{\footnotesize
\makebox[0.2\textwidth]{\raisebox{-1ex}{\hspace*{14ex}\includegraphics[width=0.08\textwidth]{Figures/vgu-logo.pdf}}}
}  % Kopfzeile rechts
...
% VGU color (from internship-report branch)
...
\hypersetup{
    ...
    filecolor=black,      
    urlcolor=black,
    pdftitle={Multi-Agent Reinforcement Learning Framework for Graph-Aware Traffic Signal Control for German Traffic Scenarios},
...
\input{TeXFiles/002_NonDisclosureNotice}
\input{TeXFiles/003_Declaration}
\input{TeXFiles/011_Acknowledgements}
\input{TeXFiles/010_Abstract}
...
%\myName
\input{TeXFiles/020_Introduction}
%\input{TeXFiles/004_ThesisFormattingGuide}      % Uncomment only while editing the template guide.
\input{TeXFiles/040_StateOfTheArt}
\input{TeXFiles/030_Background}
\input{TeXFiles/050_Methodology}
\input{TeXFiles/060_Evaluation}
\input{TeXFiles/070_Results}
\input{TeXFiles/080_Conclusions}
