...
\begin{abstract}
    ...
    We therefore study how different partitioning strategies preserve or sever inter-agent dependencies, and whether higher dependency preservation translates into better multi-agent coordination. We apply InfoMap, an information-theoretic community detection algorithm
    ...
    across three coordination coupling levels: reward shaping (IPPO), centralized
    critic (MAPPO) and graph attention (CoLight).
    We introduce the \emph{dependency retention ratio}~$\Psi$, a partition quality metric computed from mutual information and Jacobian-based dependency
    instruments, and show that InfoMap achieves higher $\Psi$ than topology-based
    partitions (METIS) across all three levels.
    Experiments in real SUMO scenarios demonstrate up to $2.85\times$ reduction in
    the average vehicle waiting time, with verified spatial advantage of MI (H1) and
    spill containment (H2) as the causal mechanism.
\end{abstract}
...
\medskip
\noindent\textbf{Theoretical grounding: why containment, not just correlation, matters.}
Spill-back containment has a formal interpretation beyond the statistical test above.
In the Lyapunov-drift framework for constrained queueing networks~\citep{tassiulas1992stability},
a decentralised scheduler preserves the stability region of a centralised one only if it
can react to the joint queue state of servers whose queues are dynamically coupled;
max-pressure signal control~\citep{varaiya2013maxpressure} instantiates exactly this
drift argument for signalised intersections via a quadratic Lyapunov function over
per-approach queue lengths.
Read against that framework, H2 states the condition under which cutting a
coordination-graph edge is safe: an edge can be severed without sacrificing the
stability that joint observation would provide only if spill-back across it is rare.
We do not derive a stability guarantee for the learned MARL policies studied here;
we invoke this literature as the control-theoretic motivation for treating spill-back
rate, rather than mutual information alone, as the criterion that should gate which
edges a partition is permitted to cut.
