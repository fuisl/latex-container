...
\newcommand{\PsiG}{\Psi_{\Gamma}}
\newcommand{\cut}[1]{c(i) \neq c(j)}

\title{The Retention Chain, in Full:\\
    \large From Stationary-Walk Escape to Regional Value Loss ---
...
Writing $\Phi_W$ for empirical route-flow retention, $\Phi_\pi$ for
stationary-walk retention, $\PsiG$ for retained influence, and
$J^\star - J_{\mathcal{P}}$ for the value lost by decomposing the MARL
problem regionally:
\begin{center}
    \small
    \begin{tabular}{lll}
        ...
        \midrule
        1. stationary walk $\Leftrightarrow$ retained flow          &
        $q_{\circlearrowleft} = 1 - \Phi_\pi$                       &
        exact identity (\S\ref{sec:link1})                         \\
        2. cut route flow bounds cut influence                      &
        $1 - \PsiG \leq \kappa (1 - \Phi_W) + \epsilon_{\mathrm{dem}}$ &
        stylized bound (\S\ref{sec:link2})                         \\
        3. cut influence bounds value loss                          &
        $J^\star - J_{\mathcal{P}} \leq C (1 - \PsiG) + \epsilon$   &
        imported, informal (\S\ref{sec:link3})                     \\
        \bottomrule
    \end{tabular}
\end{center}
Chained, the links say: \emph{a partition with low stationary-walk escape
    and high empirical route-flow retention tightens the flow-mediated
    component of the regional value-loss bound}.
...
information.
Link 2 is stated for the influence matrix $\Gamma$.
MI is a model-free audit of the mechanism and relates to influence only under
extra assumptions; small observed MI does not prove that a strong channel is
absent, since the demand distribution may leave it unexercised.
...
\begin{definition}[Retention ratios]
    For a non-negative matrix $M$ (flow, influence, or measured dependency),
    ...
    We write $\Phi_W$ for empirical route flow, $\Phi_\pi$ for the stationary
    walk, $\PsiG$ for the influence matrix $\Gamma$, and
    $\Psi(\mathcal{P},D)$ for empirical audit matrices.
    Each lies in $[0,1]$, and its complement is the cut fraction.
\end{definition}

The main dependency audit is lagged mutual information between
...
% ============================================================
\section{Link 1: the map equation is a statement about stationary-walk retention}
\label{sec:link1}
...
$q_{\circlearrowleft}$ is the cut fraction of the stationary edge measure
used by the map equation.
The empirical route-flow score $\Phi_W$ coincides with this stationary score
only when the empirical OD edge measure matches the walk's stationary edge
measure.

\begin{proposition}[$q_{\circlearrowleft} = 1 - \Phi_\pi$]
    \label{prop:phi}
    Let $\mu^\pi_{ij}=\pi_iP_{ij}$ be the stationary edge measure of the
    random walk evaluated by the map equation, and define
    ...
    \begin{equation}
        ...
        = 1 - \Phi_\pi(\mathcal{P}).
    \end{equation}
    For empirical route flow,
    $\Phi_W(\mathcal{P})=\sum_{c(i)=c(j)}\mu^W_{ij}$ with
    $\mu^W_{ij}=W_{ij}/\sum_{a,b}W_{ab}$, and the mismatch is
    $\epsilon_{\mathrm{walk}}=|\Phi_\pi-\Phi_W|$.
\end{proposition}

\begin{proof}
    The map equation counts module exits under the stationary walk.
    Summing the stationary edge mass over all cross-module edges gives
    $q_{\circlearrowleft}$; since the total stationary edge mass is one, this
    is the complement of the within-module stationary mass
    $\Phi_\pi(\mathcal{P})$.
    The empirical score uses $\mu^W$ instead, so equality requires
    $\mu^W=\mu^\pi$.
\end{proof}

\begin{corollary}[Infomap favors entropy-regularized flow retention]
    At any fixed codebook-entropy profile, $L(\mathcal{M})$ is increasing in
    $1 - \Phi_\pi$; the entropy terms are the regularizer that excludes the
    degenerate one-module solution ($\Phi_\pi = 1$ at full within-module
    description cost).
\end{corollary}
...
% ============================================================
\section{Link 2: cut flow bounds cut influence (proved in a stylized model)}
\label{sec:link2}

\subsection{Why a stylized model, and which one}
\label{sec:whymodel}

The manuscript-level claim --- flow cut by a partition upper-bounds the
influence it cuts --- is stated for $\Gamma$.
Relating this influence object to an information-theoretic audit such as MI
requires extra dynamics and distributional assumptions.
...
\begin{theorem}[Optional MI bridge for the cut-influence story; stylized model]
    \label{thm:transfer}
...
\emph{certifies} low cut influence in the manuscript object, and low cut
dependency in the stylized MI bridge (up to the demand term), but high
retention does not certify high task performance.
It is a sufficient structural certificate for tightening the coupling-loss
term, while low retention only weakens that guarantee.
...
\emph{Idealizations a critic should probe} (each is a genuine limitation; we
...
\begin{enumerate}
    ...
    \item \textbf{Gaussianity.} Used twice: the MI--correlation bijection and
          ...
          Defense: both have sub-Gaussian analogs at the price of constants;
...
% ============================================================
\section{Link 3: cut influence bounds regional value loss (imported)}
\label{sec:link3}
...
\begin{equation}
    J^\star - J_{\mathcal{P}}
    \;\leq\;
    C \left(1 - \PsiG(\mathcal{P})\right)
    + \epsilon_{\mathrm{approx}} + \epsilon_{\mathrm{opt}},
    \label{eq:valueloss}
\end{equation}
with $\Gamma$ the \emph{influence} matrix and the $\epsilon$'s collecting
...
The causal reading of Link 3 is therefore unresolved under the current demand
regime rather than established by the available traces.
Three readings survive: closed-loop damping (perturbations measured under
trained stabilizing policies at all other intersections, which attenuate
...
The constructive consequence is interpretive: if cuttable influence is near
zero for every partition, Eq.~\eqref{eq:valueloss}'s coupling term cannot
distinguish partitions, and the observed partition-dependent training
differences are consistent with $\epsilon_{\mathrm{opt}}$ ---
\emph{optimization conditioning}.
Correlated within-module signals make this explanation plausible, but they do
not by themselves prove it.
...
\begin{center}
    ...
    \begin{tabular}{p{2.6cm} p{3.4cm} p{4.2cm} p{3.6cm}}
        ...
        \addlinespace
        3 (influence $\Rightarrow$ value loss)
             & Imported, informal                               &
...
The chain's overall claim --- stationary-walk escape and empirical route-flow
retention tighten the flow-mediated component of regional value loss --- is
accordingly: exact for the stationary walk at step 1, stylized at step 2,
and conditional on imported machinery at step 3.
