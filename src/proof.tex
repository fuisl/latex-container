...
signalised intersections are partitioned into modules, and coordination
...
\begin{center}
    ...
    \begin{tabular}{lll}
        ...
        theorem in a stylised model (\S\ref{sec:link2})                                            \\
    ...
    \end{tabular}
\end{center}
Chained, the links say: \emph{a partition that minimises the map equation
    minimises an upper bound on the value loss of regional decomposition}.
...
$\mathcal{V}$ is the set of $N$ signalised intersections.
$W_{ij} \geq 0$ is the observed vehicle count travelling $i \to j$ over a
measurement window (from trip logs under a random warm-up policy);
$\Wt_{ij} = W_{ij} + W_{ji}$ is its symmetrisation.
...
Infomap searches for the partition minimising $L$.
...
detection formalised by Markov
...
% ============================================================
\section{Link 2: cut flow bounds cut dependency (proved in a stylised model)}
\label{sec:link2}

\subsection{Why a stylised model, and which one}
\label{sec:whymodel}
...
\emph{linearisation of a link-queue / cell-transmission
...
\begin{itemize}
    \item[(M1)] \textbf{Locality.} $A_{ij} \neq 0$ only if
          ...
          \emph{Reading:} one-step queue evolution depends on neighbouring
    ...
    \item[(M2)] \textbf{Flow-bounded coupling.} $|A_{ij}| \leq \alpha
          ...
          against the flow direction, hence the symmetrisation $\Wt$) --- each
    ...
    \item[(M3)] \textbf{Sub-criticality.}
          ...
          neighbours, and receives from them, is strictly below unity --- the
...
\begin{theorem}[Cut flow bounds cut dependency; stylised model]
    \label{thm:transfer}
    ...
    of $\bar{D}^{\mathrm{MI}}$, since Eq.~\eqref{eq:transfer}'s unnormalised
...
state our best defence):
\begin{enumerate}
    \item \textbf{Linearity.} Eq.~\eqref{eq:var} is a linearisation; real
          queue dynamics are piecewise (free-flow vs.\ congested branches of
          the fundamental diagram).
          Defence: the linearisation is standard for undersaturated analysis,
    ...
    \item \textbf{Gaussianity.} Used twice: the MI--correlation bijection and
          ...
          Defence: both have sub-Gaussian analogues at the price of constants;
    ...
    \item \textbf{Dual-norm sub-criticality (M3).} Row sums bound influence
          ...
          ($\rho(A) < 1$).
          Defence: physically, an intersection neither exerts nor receives
...
neighbourhoods approximate the global optimum with bias controlled by the
...
function-approximation and optimisation error the partition does not control.

\paragraph{Two formal gaps, stated precisely.}
First, the cited neighbourhood-truncation guarantees are proved for specific
...
open work.
(In the stylised linear model with quadratic queue cost this becomes a
decentralised-LQR question with block-structured information --- plausibly
...
\emph{randomised action intervention} on the frozen policies, in which the
logged random assignment is exogenous by construction.
The randomised design passed its calibration checks (negative controls at
...
fall to $\epsilon_{\mathrm{opt}}$ --- \emph{optimisation conditioning} ---
...
\begin{center}
    ...
    \begin{tabular}{p{2.6cm} p{3.4cm} p{4.2cm} p{3.6cm}}
        ...
        influence one-sidedness
             & IBA set-based route; decentralised-LQR version;
        interventional CMI                                                            \\
        \bottomrule
    \end{tabular}
\end{center}
...
The chain's overall claim --- map-equation minimisation minimises an upper
bound on regional value loss --- is accordingly: exact at step 1, proved in a
stylised model at step 2, and conditional on imported machinery at step 3.
