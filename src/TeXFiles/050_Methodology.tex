...
Figure~\ref{fig:fgs_framework} provides a side-by-side overview of the FGS and FGSv2 architectures. Both variants operate under the CTDE paradigm: the actor uses only agent $k$'s local embedding at execution time, while the centralized critic additionally receives the full graph context and the joint action-probability matrix during training.
...
The graph encoder $f_\psi$ transforms the dictionary observation into a local embedding for agent $k$ used by the actor and a full graph tensor used by the centralized critic. Given node features $\mathbf{X} \in \mathbb{R}^{N \times d}$, the encoder proceeds as follows:
\begin{enumerate}
  ...
  \item \textit{Output.} The graph tensor $\mathbf{H}^{\prime} \in \mathbb{R}^{N \times d_h^{\prime}}$ stacks all updated embeddings; the agent embedding $\mathbf{h}_k^{\prime}$ is the row indexed by agent index $k$.
\end{enumerate}

\paragraph{Module~III — Actor.}
The actor $\pi_\phi$ maps agent $k$'s embedding to a distribution over valid phases. Raw logits are produced by a linear head, then invalid phases are suppressed by setting their logits to $-\infty$ before the softmax:
\[
  \mathbf{l} = \pi_\phi(\mathbf{h}_k^{\prime}),
  \qquad
...
The critic extends the FGS design by incorporating agent $k$'s action token alongside the full graph context. For each candidate action $a$ of agent $k$, the critic input concatenates: the flattened stack of all agents' gated graph contexts $\operatorname{vec}(\mathbf{g}_1,\ldots,\mathbf{g}_N)$; the joint-action context $\mathbf{J}_k^{(a)}$ (the joint action-probability matrix with agent $k$'s row replaced by the one-hot for $a$, using hard replayed actions during TD updates and soft policy probabilities during the actor loss); the action token $\mathbf{z}_{k,a}$; the graph context $\mathbf{g}_k$; and a one-hot agent identity $\mathbf{e}_k$:
\begin{equation}
  Q_k(a) = f_Q\!\Bigl[\,
...
FGSv3 replaces the $O(N)$ centralized critic with a factored critic that aggregates only over the direct neighbors of agent $k$:
\begin{equation}
...
\EndFor
\Statex \rule{\linewidth}{0.3pt}
\Statex \textbf{Module III — Action-conditioned actor for agent $k$}
