...
\begin{table}[htbp]
    \centering
    \small
    \label{tab:WorkflowVsAgent}
    \begin{tabularx}{\textwidth}{X|X}
        \textbf{Workflow Systems}            & \textbf{Agent Systems}               \\
        \hline
        Deterministic orchestration          & Stateful reasoning across steps      \\
        Explicit retries and idempotency     & Tool invocation under uncertainty    \\
        Strong auditability and traceability & Non-deterministic (\gls{llm}-driven) \\
        Compliance and integration focus     & Evaluation, guardrails, governance   \\
    \end{tabularx}
    \caption{Workflow systems vs agent systems: key characteristics.}
\end{table}
...
observability, and control. This will inform architectural choices
and justify using different systems for different tasks.