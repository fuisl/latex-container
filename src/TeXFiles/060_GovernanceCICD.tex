...
\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.75\textwidth]{Figures/langgraph-redis-2.png}
      \caption{Redis usage counters for token tracking by organization and model type.}
      \label{fig:langgraph_redis}
\end{figure}
...
The CI/CD pipeline automates build, validation, and deployment with basic quality
gates. It is triggered on changes to the main branch and follows a linear set of
stages for clearer failure diagnosis as demonstrated in Figure~\ref{fig:cicd_pipeline}. In short: build, validate, push, deploy.

Key learning points:
\begin{itemize}
      \item Compile and validate graphs during CI to catch schema/config errors early, not at runtime.
      \item Use immutable container images to enable reproducible rollbacks and auditability.
      \item Keep pipeline stages simple and linear to make failures easy to trace.
\end{itemize}

\begin{figure}[htbp]
      \centering
      \includegraphics[width=0.75\textwidth]{Figures/pipeline.png}
      \caption{CI/CD Pipeline in development and staging environment.}
...
Key learning points:
\begin{itemize}
      \item Separate maintenance tasks from the request path to avoid latency regressions.
      \item Concurrency requires connection pooling for Redis and database access to prevent exhaustion.
      \item Serverless cold starts are acceptable for staging/early production given reduced operational burden.
\end{itemize}
...
Key learning points:
\begin{itemize}
      \item Inject environment-specific secrets at deploy time; avoid baking credentials into images.
      \item Load environment variables before higher-level configs so runtime overrides take precedence.
      \item Use one artifact across environments to reduce drift and simplify promotion.
\end{itemize}
...
Key learning points:
\begin{itemize}
      \item Human-in-the-loop evaluation remains essential; automated metrics cannot judge output utility alone.
      \item Use traces for visibility into latency and failure modes, but interpret signals in task context.
      \item Focus evaluation on correctness, stability of interrupts/resumes, and responsiveness under edits.
\end{itemize}
